...
plot.FMRM <- function(x, ...) {
  # ----error check----
  if (all(is.na(x$parameters_same_alpha))) {
    stop(
      "plot() on an object of class FMRM is invalid if model was estimated
         with penalty = FALSE\n"
    )
  }
  ...
  plot_one <- ggplot(
    ...
    theme_bw() +
    labs(
      y = toupper(x$parameters$ic_type),
    ...
    guides(alpha = "none") +
    geom_point(
      data = long[which(long[, 1] == x$parameters$alpha), ],
      na.rm = TRUE
    )
  ...
  individual_coef <- sapply(x$parameters_same_alpha, function(p) {
    as.vector(p$beta[, -1])
  })
  ...
  plot_two <- ggplot(
    ...
    theme_bw() +
    labs(
      y = expression("Coefficients: " * beta),
      x = expression(log(lambda)),
      color = "Parameters"
    ) +
    theme(text = element_text(family = "serif", face = "bold", size = 12))
  ...
  # ----optimal alpha----
  group_norm <- sapply(x$parameters_same_alpha, function(p) {
    sqrt(rowSums(p$beta[, -1]^2))
  })
