...
print_model_FMRM <- function(
  ...
  cat(
    " lambda =",
    round(selected_parameters$lambda, 2),
    "|| alpha =",
    selected_parameters$alpha,
    "|| log-likelihood =",
    round(selected_parameters$loglik, 2),
    "|| ",
    toupper(information_criteria),
    " =",
    round(selected_parameters$ic, 2),
    "|| MSE =",
    round(selected_parameters$mse, 2),
    "\n\n"
  )
  ...
  cat(paste(sprintf("%6.0f", idx), collapse = " "))
  cat("\n  Intercept   ")
  cat(paste(sprintf("%6.3f", selected_parameters$beta[, 1]), collapse = " "))
  for (k in 2:ncol(selected_parameters$beta)) {
    cat("\n  Beta", k - 1, "     ")
    cat(paste(sprintf("%6.3f", selected_parameters$beta[, k]), collapse = " "))
  }
  cat("\n\n")
  cat(strrep("-", getOption("width")), "\n")
}
