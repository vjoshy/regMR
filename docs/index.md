...
mixture regression models via an MM algorithm. The sparse group lasso
...
fit models over different lambda-alpha sgl penalties and group counts.
There are four families of finite mixture regression models able to be
fit: Gaussian, Poisson, Binomial, and Gamma. These are specified using
the `family` argument in the three main functions the package contains,
[`FMRM()`](https://www.vinayjoshy.ca/regMR/reference/FMRM.md),
[`MM_Grid()`](https://www.vinayjoshy.ca/regMR/reference/MM_Grid.md), and
[`MM()`](https://www.vinayjoshy.ca/regMR/reference/MM.md). The models
can also be selected using different information criteria including the
default Bayesian Information Criterion (BIC), group-structured Extended
BIC (gEBIC), Akaike Information Criterion (AIC), and Integrated
Classification Likelihood (ICL) Criterion. See the documentation for
more information regarding how the arguments are specified.

This readme file provides a brief and basic example of how to use the
regMR package. It walks through generating clustered data to which a
finite Gaussian mixture regression model is fit, applying
[`FMRM()`](https://www.vinayjoshy.ca/regMR/reference/FMRM.md) to carry
out that procedure over different lambda-alpha sgl penalties and group
counts, and applying the plotting and summary methods/functions to the
model to get the most use out of the package.
...
generated using the
[`rmvnorm()`](https://rdrr.io/pkg/mvtnorm/man/Mvnorm.html) function from
`mvtnorm`.

`y`: To generate `y`, the group responsibilities and mean vector must be
initialized. For the group responsibilities, we use the
[`rmultinom()`](https://rdrr.io/r/stats/Multinom.html) function coupled
with the $`\pi`$ vector. For the mean vector, we calculate
$`\beta_0 + \sum^p_{j = 1}x_{ij}\beta_{gj}`$, where i goes from 1 to n
and g is the group that the $`i^{th}`$ observation belongs to. Then,
using the mean vector and the true standard deviations, `y` (a response
vector of length n) is generated with
...
We call [`FMRM()`](https://www.vinayjoshy.ca/regMR/reference/FMRM.md)
with `family = gaussian()` and `information_criteria = "bic"` to fit a
finite Gaussian mixture regression model to the data.
[`FMRM()`](https://www.vinayjoshy.ca/regMR/reference/FMRM.md) fits
regularized finite mixture regression models via the MM algorithm over a
range of lambda-alpha sgl penalties and group counts. The maximum group
count to be tested is specified in the function below as `G = 4`. The
function chooses the model with the lowest information criteria value as
...
[`plot()`](https://rdrr.io/r/graphics/plot.default.html) is an S3 method
for plotting results from the
[`FMRM()`](https://www.vinayjoshy.ca/regMR/reference/FMRM.md) and
[`MM_Grid()`](https://www.vinayjoshy.ca/regMR/reference/MM_Grid.md)
functions of class FMRM. The function outputs three plots:
...
![](reference/figures/README-example-3-2.png)
...
![](reference/figures/README-example-3-3.png)

[`plot2()`](https://www.vinayjoshy.ca/regMR/reference/plot2.md) plots
two specified covariates of the predictor/design matrix (`X`) against a
response vector (`y` or `y_hat`). The observations are coloured per the
group responsibility matrix (`z_hard`) in the finite mixture regression
...
![](reference/figures/plot2_2.png)

[`summary()`](https://rdrr.io/r/base/summary.html) is an S3 method for
summarizing results from the
[`FMRM()`](https://www.vinayjoshy.ca/regMR/reference/FMRM.md) and
[`MM_Grid()`](https://www.vinayjoshy.ca/regMR/reference/MM_Grid.md)
functions of class FMRM. Outputs the number of mixture components,
...
## Computational Details and Benchmarking
...
y <- cbind(successes, failures = m - successes)
```
