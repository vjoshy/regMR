...
Compute group responsibility matrix for finite mixture regression
...
- ...:
  ...
  Additional arguments for computing the group responsibilities
  depending on the inputted family.

## Value

A numeric matrix of size n x G, where the number of rows is equal to the
number of observations n, and the number of columns is equal to the
number of mixture components G, representing the group responsibilities
for the given model.
